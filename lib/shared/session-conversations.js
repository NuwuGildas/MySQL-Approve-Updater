...
function createSessionConversations({ dataDir, log = () => {}, httpError = fail, proposals = [] } = {}) {
  ...
  let readOnly = null;      // the file could not be READ: nothing may be written over it
  let lastSaveError = null; // a write failed: the data is intact, the copy on disk is behind
  ...
  /* A write that failed is NOT the same thing as a file that cannot be read.
     The conversation in memory is intact; only the copy on disk is behind, and the next message
     writes the whole document again, so the way out is to keep going rather than to stop. Latching
     this into readOnly - which is what it used to do - turned one momentary lock on Windows into a
     store that refused every write for the life of the process, and the user got a 503 for each
     thing they typed afterwards. The failure is still reported; it just is not fatal. */
  function save() {
    writable();
    const snapshot = JSON.stringify(data);
    if (require('../persistence/runtime').current()?.mode === 'mysql') {
    ...
    const work = chain.then(async () => {
      ...
      if (lastSaveError) { lastSaveError = null; log('info', `${FILE_NAME} was saved again`); }
    });
    chain = work.catch((error) => {
      lastSaveError = `${FILE_NAME} could not be saved: ${error.message}`;
      log('error', `${lastSaveError} The conversation is safe in memory and the next message will try again.`);
    });
    return work;
  }
  ...
  function status(id) {
    ...
    return {
      ...
      host: s.host, user: s.user, since: s.createdAt, readOnly, lastSaveError,
