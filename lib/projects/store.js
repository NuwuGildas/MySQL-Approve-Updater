...
/* Project store: groups the reusable resources of this app (DB connections, SSH servers, git
   ...
   (lib/shared/atomic-file, like connections.json and the deploy stores). A project only ever
...
const { writeFileAtomic } = require('../shared/atomic-file');
...
function createProjectStore(DATA_DIR, { log = () => {} } = {}) {
  ...
  const bindDeployments = (access) => { deployments = access; };
  const resourcesFor = (p) => ({ ...p.resources, ...(deployments ? { targets: deployments.targets().filter((t) => t.projectId === p.id).map((t) => t.id) } : {}) });
  ...
  function save() {
    ...
    const p = chain.then(() => writeFileAtomic(file, snapshot));
  ...
  async function remove(id) {
    ...
    if (deployments && (deployments.targets().some((t) => t.projectId === id) || deployments.runs().some((r) => r.projectId === id) || deployments.pending?.(id))) {
      throw fail(409, 'This project has deployments, deployment history, or a deployment being created and cannot be deleted');
    }
  ...
  async function link(id, kind, resourceId) {
    ...
    const p = must(id);
    if (kind === 'targets' && deployments) {
      const target = deployments.targets().find((t) => t.id === resourceId);
      if (!target) throw fail(404, 'Deployment not found');
      if (target.projectId !== id) throw fail(409, 'A deployment belongs to one project. Change its project in the deployment settings');
      return p;
    }
  ...
  async function unlink(id, kind, resourceId) {
    assertWritable(); assertKind(kind); assertResourceId(resourceId);
    const p = must(id);
    if (kind === 'targets' && deployments?.targets().some((t) => t.id === resourceId && t.projectId === id)) {
      throw fail(409, 'A deployment must always belong to a project. Change its project in the deployment settings');
    }
  ...
  function projectsFor(kind, resourceId) {
    assertKind(kind);
    return data.projects.filter((p) => (resourcesFor(p)[kind] || []).includes(resourceId));
  }

  /* ---- scope: what a project may see ----
     Attaching a resource to a project takes it OUT of the shared pool: from
     then on it exists only inside the projects that claim it, until it is
     detached again. A resource no project claims is unassigned and stays
     visible everywhere, which is what makes attaching a deliberate act rather
     than a prerequisite for using anything.

     This is the single definition of the rule. Everything that lists resources
     - the base, every module backend, the assistant's tools - decides through
     it, so no two surfaces can disagree about what a project contains. */

  /** Is `resourceId` of `kind` part of `projectId`'s world? */
  function visibleIn(projectId, kind, resourceId) {
    assertKind(kind);
    const claims = projectsFor(kind, resourceId);
    if (!claims.length) return true;                       // unassigned: shared by every project
    return claims.some((p) => p.id === projectId);
  }

  /**
   * A reusable filter for one project. `id` pulls the resource id out of an
   * item, so callers can filter plain ids or whole records.
   *   const scope = store.scopeFor(projectId);
   *   scope.filter('servers', profiles, (p) => p.id)
   */
  function scopeFor(projectId) {
    // Resolve the claims once per kind: listing is the hot path here.
    ...
    const claimsOf = (kind) => {
      ...
      if (!claimed.has(kind)) {
        ...
        for (const p of data.projects) {
          for (const rid of resourcesFor(p)[kind] || []) (p.id === projectId ? mine : others).add(rid);
        }
        claimed.set(kind, { mine, others });
      }
      return claimed.get(kind);
    };
    const visible = (kind, resourceId) => {
      const { mine, others } = claimsOf(kind);
      return mine.has(resourceId) || !others.has(resourceId);
    };
    return {
      projectId,
      visible,
      /** Why a resource is not here, for a message the user can act on. */
      ownersOf: (kind, resourceId) => projectsFor(kind, resourceId).map((p) => ({ id: p.id, name: p.name })),
      filter: (kind, items, id = (x) => x) => items.filter((item) => visible(kind, id(item))),
    };
  }
  ...
  async function unlinkEverywhere(kind, resourceId) {
    ...
    if (kind === 'targets' && deployments?.targets().some((t) => t.id === resourceId)) throw fail(409, 'A deployment must always belong to a project');
