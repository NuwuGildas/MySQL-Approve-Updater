...
function mount(rawCtx) {
  ...
  const stores = createStores(ctx.DATA_DIR, { log: ctx.logEvent });
  ...
  engine.onEvent((type, run) => {
    ...
    if (!ctx.settings.aiAssist?.autoExplain || !ctx.agent.isConnected()) return;
  ...
  ctx.app.use('/api/deploy', createRouter({ ctx, engine, stores, vault, redact, agentApi, autoShip, cloud, config }));
  const connectors = createConnectors({ ctx, stores, vault });
  ctx.app.use('/api/connectors', connectors.router); // git provider accounts (GitHub, GitLab)
  ...
  return { engine, stores, vault, redact, agentApi, autoShip, cloud, config, connectors };
}
