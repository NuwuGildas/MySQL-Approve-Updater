...
function register({ ctx, engine, stores, vault, redact }) {
  ...
  function proposeAction({ targetId, action, release, reason, source }) {
    ...
    const prop = { id: crypto.randomUUID(), kind: 'deploy-action', action, targetId: t.id, targetName: t.name, release: release || null, reason: String(reason || '').slice(0, 400), planHash: latestPlan ? (latestPlan.plan?.hash || latestPlan.planSummary?.hash) : null, source: source || 'agent', status: 'pending', ts: new Date().toISOString() };
    ctx.agent.proposals.push(prop);
    while (ctx.agent.proposals.length > 30) ctx.agent.proposals.shift();
  ...
  async function runToChat(runId, { lines = 150 } = {}) {
    ...
    ctx.agent.chatNote({ kind: 'run-log', runId: run.id, targetId: run.targetId, target: run.targetName, mode: run.mode, status: run.status, error: run.error || null, stage: failing?.name || run.stage || null, lines: log.map((l) => `[${l.stage}/${l.stream}] ${l.line}`),
    ...
    return { runId: run.id, lines: log.length };
  }
...
Propose an action only when it would actually help now: "plan" after a config/manifest fix to re-verify, "ship" to retry a transient failure, "rollback" when the live release is broken and a previous one exists, "unlock" when a stale deploy lock blocks the target, "cancel" when a run is stuck. Otherwise use "none". The user approves every action before it runs.`;
  ...
  async function preShipReview(targetId) {
    ...
    ctx.agent.chatNote({ kind: 'deploy-preship', targetId: t.id, text: `Pre-ship review of "${t.name}": ${rec.verdict.toUpperCase()}: ${rec.summary}${rec.findings.length ? '\n' + rec.findings.map((f) => `- [${f.level}] ${f.text}`).join('\n') : ''}` });
    return rec;
  }
