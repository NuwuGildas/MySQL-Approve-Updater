    ...
    (async () => {
      try {
        ...
        if (spec.createTarget && createTarget) {
          const t = await createTarget({ name: spec.createTarget.targetName, repoId: spec.createTarget.repoId, type: 'vps-ssh', buildMode: 'auto', ssh: { profileId: profile.id }, paths: { root: `/var/www/${spec.createTarget.targetName.toLowerCase().replace(/[^a-z0-9_.-]+/g, '-')}` }, web: { server: 'nginx', reloadCmd: 'sudo -n /usr/bin/systemctl reload nginx', phpFpmReload: spec.recipe === 'php' ? 'sudo -n /usr/bin/systemctl reload php8.3-fpm' : '' }, process: { manager: ['node', 'python'].includes(spec.recipe) ? 'systemd' : 'none', unit: ['node', 'python'].includes(spec.recipe) ? `${spec.createTarget.targetName}.service` : '' }, healthUrl: spec.createTarget.healthUrl || `http://${rec.ip}/`, keepReleases: 5 });
