...
function createRouter({ ctx, engine, stores, vault, redact, agentApi, autoShip, cloud, config }) {
  ...
  r.post('/targets/test', wrap(async (req, res) => {
    ...
    try {
      ...
      let dir = null;
      const dirPath = norm.type === 'vps-ssh' ? norm.paths?.root : norm.type === 'shared-hosting' ? norm.paths?.docroot : null;
      if (dirPath && typeof conn.exists === 'function') { try { dir = { path: dirPath, exists: await conn.exists(dirPath), writable: norm.type === 'vps-ssh' ? probe.rootWritable ?? null : null }; } catch { dir = { path: dirPath, exists: null, writable: null }; } }
