...
/* A reply that says it cannot call a tool it WAS given.
 *
 * This application does not register its tools with the provider's CLI; it lists them in the system
 * prompt and reads back a {"tool":...} object. A model running inside a harness that has its own
 * tool registry looks for ssh_status there, does not find it - it was never going to be there - and
 * tells the user the tools "are only described in prose" and "aren't callable". They are callable.
 * It just had to emit the JSON.
 *
 * The discriminator is the tool NAME. Saying "deployments are on another page, I have no tool for
 * that here" is correct and expected under page scoping; saying it about `ssh_exec` while holding
 * `ssh_exec` is the mistake. So this only fires when the denial names a tool the conversation was
 * actually handed.
 */
const DENIAL_WORDS = [
  /\bnot? (?:actually )?(?:registered|callable|invocable|available to me)\b/i,
  /\b(?:aren'?t|isn'?t|are not|is not) (?:actually )?(?:registered|callable|invocable|available|exposed)\b/i,
  /\bonly (?:described|documented|mentioned) in (?:prose|text|the prompt)\b/i,
  /\bno matching (?:deferred )?tools?\b/i,
  /\bcannot (?:actually )?(?:call|invoke|use) (?:it|them|this|that|those)\b/i,
  /\bI (?:do not|don'?t) (?:actually )?have (?:access to|a way to call)\b/i,
  /\bnot (?:in|part of) (?:my|the) (?:tool )?(?:registry|list|harness)\b/i,
];
/**
 * @param {string} text the model's reply
 * @param {string[]} toolNames the tools this conversation was given
 */
function deniesAvailableTool(text, toolNames = []) {
  const body = String(text || '');
  if (!DENIAL_WORDS.some((pattern) => pattern.test(body))) return null;
  // Only a tool it HAS. A refusal about something it was never given is the right answer.
  return toolNames.find((name) => name && new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(body)) || null;
}
const DENIED_TOOL_GUIDANCE = (name) => `TURN GUIDANCE: your last reply told the user that ${name} is not registered, not callable, or only described in prose. That is wrong, and the user cannot act on it. ${name} IS available to you right now. These tools are not registered with the harness you are running inside and will never appear in its tool registry or any tool search - there is nothing to look them up in, and looking is what produced this mistake. You call one by replying with ONLY the JSON object {"tool":"${name}","input":{...}} and no other text; this application reads it, runs the tool, and sends you the result. Call the tool now, or answer the question without claiming you lack a tool you were given.`;
...
/* The last step of a turn is always an answer.
 *
 * A turn is a budget of tool calls, and running out of them used to end the turn with a canned
 * apology - discarding everything the tools had just returned, and saying "I did not finish" even
 * when the work WAS done and a card was sitting in front of the user. The budget now buys tool
 * calls; the reply is not one of them. */
const OUT_OF_STEPS_GUIDANCE = 'TURN GUIDANCE: you have used every tool call for this turn, so this reply must be your answer, in prose. Do not call another tool. Report what you found and what you did with the results you already have, including anything you proposed. If you genuinely could not finish, say what is still missing and what you would need to do next.';
...
function createAgentWorkflow(ctx) {
  ...
  /* The page is what the user is looking at. It narrows the tool list and shapes the instructions, so
     a question asked on the MySQL Update Tool is answered with rules and the database rather than
     with everything the workspace can do. */
  function buildPrompt(sessionId, { moduleNote, transcriptExtra, guidance, page }) {
    const recent = sessions.history(sessionId).slice(-HISTORY_WINDOW);
    return model.systemPrompt(sessionId, page) + (moduleNote || '') + '\n\n--- Conversation ---\n' +
  ...
  async function turn(hold, sessionId, { message, guidance = '', moduleNote = '', page = null }) {
    ...
    /* Cards this turn raised.
     *
     * A proposal that names a session belongs to that one. A proposal with NO session belongs to
     * whoever raised it - rule and deploy proposals are not session-bound, and the chat window says
     * so where it decides which cards to draw. Requiring a match here meant a rule proposal was
     * never reported to the client that asked for it: the tool ran, the card sat pending on the
     * server, and the user was shown nothing to approve. Everything here is already restricted to
     * proposals that did not exist when this turn started, so an unbound one is unambiguously ours. */
    const raisedByThisTurn = (p) => !propBefore.has(p.id) && (p.sessionId == null || p.sessionId === sessionId);
    const pendingNow = () => agent.proposals.filter((p) => raisedByThisTurn(p) && p.status === 'pending');
    const stopped = (extra = {}) => {
    ...
    let transcriptExtra = '', reply = null, narrated = false, deniedTools = false;
    try {
      /* maxSteps is the TOOL budget; the extra pass is the answer, which every turn is entitled to
         whether or not the budget ran out. */
      for (let step = 0; step <= maxSteps; step++) {
        const answerOnly = step === maxSteps;
        events.say({ type: 'step', step: step + 1, msg: answerOnly ? 'no tool calls left: writing the answer' : `thinking with ${model.label()}` });
        const prompt = buildPrompt(sessionId, { moduleNote, transcriptExtra, guidance: answerOnly ? OUT_OF_STEPS_GUIDANCE : guidance, page });
        ...
        // on the answering pass a tool call is not an option, so it is not looked for
        const call = answerOnly ? null : model.parseToolCall(outRaw, sessionId, page);
        ...
        /* A reply that refuses a tool it was HANDED. The user is being told the assistant cannot do
           something it can, which reads as a dead end rather than as a mistake. Same treatment as a
           narrated proposal: say what is wrong, once, and do not charge it a tool call. */
        const denied = !call && !deniedTools && !answerOnly
          ? deniesAvailableTool(outRaw, model.toolNames?.(sessionId, page) || [])
          : null;
        if (denied) {
          deniedTools = true;
          guidance = DENIED_TOOL_GUIDANCE(denied);
          log('warn', `AI chat: the model said ${denied} was not callable; telling it how to call it`);
          events.say({ type: 'step', step: step + 1, msg: `that refused ${denied}, which it does have` });
          step--; // one-shot, guarded by `deniedTools`
          continue;
        }
        if (!call && !narrated && !answerOnly && describesAnUnmadeProposal(outRaw, pendingNow().length)) {
          narrated = true;
          ...
          step--; // one-shot (guarded by `narrated`): correcting a mistake should not cost a tool call
          continue;
        }
    ...
    /* Last resort: the answering pass produced nothing usable (an empty reply, or another tool call
       despite being told there were none left). Say what actually happened rather than only that a
       limit was hit - and never call a turn unfinished when it left a card in front of the user. */
    if (reply == null || !reply.trim() || /^\s*(```(json)?\s*)?\{\s*"tool"\s*:/.test(reply)) {
      const used = [...new Set(actions.filter((a) => a.ok).map((a) => a.tool))];
      const raised = pendingNow().length;
      reply = raised
        ? `I used all ${maxSteps} tool steps for this turn and ran out before writing a summary — but the work is done: ${raised === 1 ? 'the approval card above is real' : `the ${raised} approval cards above are real`}. Review ${raised === 1 ? 'it' : 'them'} as usual${used.length ? `. Steps used: ${used.join(', ')}.` : '.'}`
        : `I used all ${maxSteps} tool steps for this turn without reaching an answer${used.length ? ` (${used.join(', ')})` : ''}. Ask again more specifically, or in smaller pieces, and I will get further.`;
    }
  ...
  async function decide(sessionId, proposalId, body = {}) {
    ...
    try {
      ...
      const continuation = await continueAfter(sessionId, hold, choice, alternative,
        body.module ? `\n\nContext: the user is currently in the "${String(body.module).slice(0, 60)}" module: tailor your help to it.` : '',
        body.page || null); // the decision resumes the turn on the page the card was raised from
  ...
  async function continueAfter(sessionId, hold, choice, alternative, moduleNote, page) {
    if (!model.connected()) {
    ...
    const options = { hold, guidance: GUIDANCE[choice], page };
...
module.exports = { createAgentWorkflow, agentEventForViewer, auditEntryForViewer, redactAuditEntry, deniesAvailableTool, AUDIT_CONTENT_FIELDS, GUIDANCE, MAX_STEPS };
