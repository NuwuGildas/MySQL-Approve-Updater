...
function createAgentWorkflow(ctx) {
  ...
  function buildPrompt(sessionId, { moduleNote, transcriptExtra, guidance }) {
    ...
    return model.systemPrompt() + (moduleNote || '') + '\n\n--- Conversation ---\n' +
  ...
  async function turn(hold, sessionId, { message, guidance = '', moduleNote = '' }) {
    ...
    try {
      for (let step = 0; step < maxSteps; step++) {
        ...
        const call = model.parseToolCall(outRaw);
        ...
        const t0 = Date.now();
        let result, ok = true;
        try { result = await agent.tools[call.tool].run(call.input || {}); }
    ...
    try { sessions.noteTurn?.(message || null, reply); } catch {} // remember this exchange for the attached server
