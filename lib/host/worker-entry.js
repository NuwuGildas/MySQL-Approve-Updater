...
function makeHostApi(info) {
  return {
    storage: require('../persistence/runtime').current(),
    id: info.id,
...
async function activate(info) {
  require('../persistence/runtime').configure({ root: info.appDataDir, role: `module:${info.id}` });
  ...
  if (require('../persistence/runtime').current().mode === 'mysql' && implementation.storageVersion !== 1) {
    throw new Error(`${info.id} must be updated to a database-compatible package before it can run with APP_STORAGE=mysql`);
  }
...
async function shutdown(reason) {
  ...
  await require('../persistence/runtime').current()?.close();
...
process.on('message', async (message) => {
  ...
  switch (message.k) {
    ...
    case 'call': {
      ...
      try {
        require('../persistence/runtime').current()?.assertHealthy();
        ...
        const value = await handler(params, meta || {});
        require('../persistence/runtime').current()?.assertHealthy();
        send({ k: 'result', id, ok: true, value });
