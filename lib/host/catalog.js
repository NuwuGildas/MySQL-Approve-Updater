...
function createCatalogClient({ registries = [], hostSdkVersion, ttlMs = DEFAULT_TTL_MS, log = () => {} } = {}) {
  /* `registries` may be a function, and then it is asked again on every refresh.
     Where a catalog comes from can change while the application is running - a
     developer builds a local one, or deletes it and should fall back to the
     published one - and resolving it once at startup made that need a restart. */
  const resolveSources = () => (typeof registries === 'function' ? registries() : registries)
    .map((r) => (typeof r === 'string' ? { name: 'Modules', url: r } : r))
  ...
  async function load({ force = false } = {}) {
    ...
    if (!resolveSources().length) { cache = { at: Date.now(), modules: [], failures: [] }; return cache; }
    return refresh();
  }
  ...
  // `sources` stays a property, and is whatever it resolves to right now.
  return { get sources() { return resolveSources(); }, load, refresh, entry, pick, cached: () => cache };
}
