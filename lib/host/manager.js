...
/**
 * Whether a module's assistant contributions may be bridged into the host.
 *
 * The manifest is what the user read and agreed to before installing. Giving the assistant new
 * tools, or new things to ask their approval for, is precisely the kind of power that list exists
 * to describe - so a module that did not ask for it does not get it, however its backend is
 * written. Registration itself used to be unguarded and only the host SERVICES behind a tool were
 * checked, which meant a tool could appear in the assistant's list and then fail the moment the
 * assistant used it.
 *
 * @param {string[]} capabilities what the manifest declares
 * @param {{assistantTools?: any[], proposalKinds?: any[]}} offered what the worker announced
 * @returns {{tools: boolean, kinds: boolean, warnings: string[]}}
 */
function assistantGrants(capabilities, offered = {}) {
  const granted = new Set(capabilities || []);
  const warnings = [];
  const may = (capability, count, what) => {
    if (!count) return false;
    if (granted.has(capability)) return true;
    warnings.push(`offers ${count} ${what}${count === 1 ? '' : 's'} but its manifest does not declare "${capability}"; ignored`);
    return false;
  };
  return {
    tools: may('assistant:tools', (offered.assistantTools || []).length, 'assistant tool'),
    kinds: may('assistant:proposals', (offered.proposalKinds || []).length, 'proposal kind'),
    warnings,
  };
}
...
function createModuleHost({
  ...
  function bindAssistant(id, worker) {
    ...
    const grants = assistantGrants(worker.state.record?.manifest?.capabilities, worker.state);
    for (const warning of grants.warnings) log('warn', `modules: ${id} ${warning}`);
    for (const tool of grants.tools ? worker.state.assistantTools : []) {
      if (!tools) break;
      ...
      tools[tool.name] = {
        ...
        /* Which pages the tool belongs on, so the assistant is offered what the user is looking at
           rather than everything at once. A module needs no extra declaration: its manifest already
           says which pages it owns. One that owns none is offered everywhere, as before. */
        pages: worker.state.record?.manifest?.pages?.length ? [...worker.state.record.manifest.pages] : undefined,
    ...
    for (const kind of grants.kinds ? worker.state.proposalKinds : []) {
      if (!kinds) break;
...
module.exports = { createModuleHost, assistantGrants, HOST_SDK_VERSION };
