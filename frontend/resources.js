/* Assigning reusable resources to a project.

   Catalogs come from whichever modules are installed: connections are the base
   application's, servers come from Servers, repositories and deployments from
   Deployments. A catalog whose module is absent is simply not offered - the
   links already stored for it are untouched and come back when it returns. */
'use strict';

export function createResourcesDialog({ host, mount }) {
  const { esc, toast } = host.ui;
  const $ = (id) => mount.querySelector('#' + id);
  const hostOn = (target, ...rest) => (target ? host.on(target, ...rest) : null);

  /** A catalog served by another module, or an empty one when it is not installed. */
  async function catalogOf(kind) {
    const servers = host.consume('servers');
    const connectors = host.consume('connectors');
    const deployments = host.consume('deployments');
    if (kind === 'servers') return servers ? servers.serverProfiles() : [];
    if (kind === 'connectors') return connectors ? { connectors: connectors.list(), providers: [] } : { connectors: [], providers: [] };
    if (kind === 'repos') return deployments ? { repos: deployments.repos() } : { repos: [] };
    if (kind === 'targets') return deployments ? { targets: deployments.targets() } : { targets: [] };
    return [];
  }
  ...
  const PR_CATALOGS = {
    connections: async () => (await host.api('/api/connections')).profiles.map((p) => ({
      id: p.id, name: p.name,
    ...
    servers: async () => (await catalogOf('servers')).map((s) => ({
      id: s.id, name: s.name, detail: `${s.user ? s.user + '@' : ''}${s.host || ''}${s.port && s.port !== 22 ? ':' + s.port : ''}`, live: s.connected,
    })),
    connectors: async () => { const d = await catalogOf('connectors'); const label = (k) => d.providers?.find((p) => p.id === k)?.label || k; return (d.connectors || []).map((c) => ({
    ...
    repos: async () => ((await catalogOf('repos')).repos || []).map((r) => ({
    ...
    targets: async () => ((await catalogOf('targets')).targets || []).map((t) => ({
  ...
  async function openProjectResources(projectId) {
    ...
    pr.projectId = projectId || host.projects.activeId();
  ...
  function prRender() {
    ...
    if (!shown.length) {
      ...
      listHost.innerHTML = `${hint}${kindErr}<div class="empty-block"><b>${q ? 'No match' : 'Nothing here'}</b>${why}${q ? '<br><button type="button" id="prClearSearch">Clear search</button>' : ''}</div>`;
      hostOn($('prClearSearch'), 'click', () => { pr.query = ''; $('prSearch').value = ''; prRender(); $('prSearch').focus(); });
      return;
    }
    ...
    listHost.innerHTML = hint + kindErr + `<ul class="pr-rows" role="list">` + shown.map((r) => {
    ...
    listHost.querySelectorAll('.pr-row [data-act]').forEach((b) => b.addEventListener('click', async () => {
      const li = b.closest('.pr-row');
      if (b.dataset.act === 'edit-deployment') {
        closeProjectResources();
        host.navigate(`#/deployments/targets/${encodeURIComponent(li.dataset.id)}`);
  ...
  async function prToggle(kind, resourceId, link) {
    ...
    try {
      const updated = link
        ? await host.rpc('link', { id: p.id, kind, resourceId })
        : await host.rpc('unlink', { id: p.id, kind, resourceId });
  ...
  function prApplyProject(updated) {
    ...
    host.projects.publish(pr.projects);
  ...
  (function wireProjectResources() {
    const d = prModal(); if (!d) return;
    hostOn($('btnProjResources'), 'click', () => openProjectResources());
    host.on($('prClose'), 'click', closeProjectResources);
    host.on($('prProject'), 'change', (e) => { pr.projectId = e.target.value; pr.busy.clear(); prRender(); });
    let t;
    host.on($('prSearch'), 'input', (e) => { pr.query = e.target.value; clearTimeout(t); t = setTimeout(prRender, 80); });
    host.on($('prRefresh'), 'click', () => { prRenderLoading(); prLoad(); });
    host.on(d, 'close', () => { pr.opened = false; pr.busy.clear(); });
    // follow the header switcher while the dialog is closed, so the next open shows the active project
    host.on(document, 'st:project', (e) => { if (!pr.opened && e.detail?.id) pr.projectId = e.detail.id; });
    /* opened through the module's own API, never through a global */
  })();

  return { open: openProjectResources };
}
