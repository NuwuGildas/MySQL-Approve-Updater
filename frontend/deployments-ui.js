/* The Ascension: build → deploy → ship.
   ...
   Moved out of the base application with its seams changed: every call goes to
   this module's own backend through the host dispatcher, the project context and
   the assistant window stay host-owned, and every listener is registered through
   the host so removing the module takes all of them with it. */
'use strict';

export function createDeploymentsUI({ host, mount }) {
  ...
  /* Servers is optional: without it there are simply no SSH profiles to pick. */
  const serverProfiles = async () => {
    const servers = host.consume('servers');
    return servers ? { sessions: await servers.serverProfiles() } : { sessions: [] };
  };
  ...
  const dpProjects = () => host.projects.list().length ? host.projects.list() : dp.projects;
  const dpProjectName = (id) => dpProjects().find((p) => p.id === id)?.name || id || 'Project unavailable';
  function dpFillProjectSelect(id, selected) {
    const fill = (list) => {
      const preferred = selected || (host.projects.activeId() || 'general');
    ...
    host.projects.load().then((d) => { dp.projects = Array.isArray(d.projects) ? d.projects : []; fill(dp.projects); }).catch(() => {});
  }
  ...
  async function loadDeploy() {
    try {
      const [st, repos, targets, secrets, sessions, runs, servers, templates, projectData] = await Promise.all([
        http('/deploy/status'), http('/deploy/repos'), http('/deploy/targets'), http('/deploy/secrets'), serverProfiles(), http('/deploy/runs?limit=100'), http('/deploy/cloud/servers').catch(() => ({ servers: [] })), http('/deploy/templates').catch(() => ({ templates: [] })), host.projects.load(),
      ]);
      ...
      host.projects.publish(dp.projects);
  ...
  host.on($('deployNav'), 'click', async (e) => {
    ...
    if (rm) {
      ...
      if (!(await confirm({ title: 'Delete secret', message: `Delete <b>${esc(name)}</b> from the vault? Anything referencing it will fail to deploy.`, okLabel: 'Delete', okClass: 'reject' }))) return;
      try { await http(`/deploy/secrets/${encodeURIComponent(name)}`, { method: 'DELETE' }); toast('Secret deleted', 'success'); loadDeploy(); } catch (err) { toast(err.message, 'error'); }
      return;
    }
    ...
    if (destroy) {
      ...
      if (!(await confirm({ title: 'Destroy server', message: `Destroy <b>${esc(sv.name)}</b> (${esc(sv.provider)}, ${esc(sv.ip || 'no ip')}) at the provider? This deletes the VM and its data. The SSH profile and target stay and will fail until you remove them.`, okLabel: 'Destroy VM', okClass: 'reject' }))) return;
      try { await http(`/deploy/cloud/servers/${sv.id}`, { method: 'DELETE' }); toast('Server destroyed', 'success'); loadDeploy(); } catch (err) { toast(err.message, 'error'); }
      return;
    }
  ...
  host.on($('dpTabs'), 'click', (e) => { const b = e.target.closest('[data-tab]'); if (b) dpShowTab(b.dataset.tab); });
  ...
  async function wzEnterFramework() {
    if (!wz.catalog) { try { wz.catalog = await http('/deploy/frameworks'); } catch (e) { toast(e.message, 'error'); return; } }
    ...
    if (dir && wz.detected?.dir !== dir) {
      ...
      try {
        const det = await http('/deploy/detect-path', { method: 'POST', body: JSON.stringify({ path: dir }) });
        wz.detected = { dir, ...det };
  ...
  async function wzAiDetect(ev) {
    ...
    try {
      const det = await http('/deploy/detect-source', { method: 'POST', body: JSON.stringify(body) });
  ...
  host.on($('btnFwAi'), 'click', wzAiDetect);
  function wzRenderFw() {
  ...
  function wzRenderReview() {
    ...
    host.on($('btnWzProjectChange'), 'click', () => { wzShow(1); $('wzProject').focus(); });
  }
  host.on($('wzSourceKind'), 'click', (e) => { const b = e.target.closest('button'); if (b) { wz.source = b.dataset.v; wzSync(); } });
  host.on($('wzDestKind'), 'click', (e) => { const b = e.target.closest('button'); if (b) { wz.dest = b.dataset.v; wzSync(); } });
  ...
  host.on($('wzTargetName'), 'input', () => { $('wzTargetName').dataset.touched = '1'; wzSync(); });
  host.on($('wzRoot'), 'input', () => { $('wzRoot').dataset.touched = '1'; });
  host.on($('wzLocalRoot'), 'input', () => { $('wzLocalRoot').dataset.touched = '1'; });
  host.on($('dtLocalRoot'), 'input', () => { tfState.localRootTouched = true; });
  host.on($('btnWzCancel'), 'click', () => $('dpWizard').close());
  host.on($('btnWzBack'), 'click', () => wzShow(Math.max(1, wz.step - 1)));
  host.on($('btnWzNext'), 'click', () => {
  ...
  async function wzFinishCloud() {
    ...
    try {
      let repoId = c.repoId;
      if (!repoId) { const r = await http('/deploy/repos', { method: 'POST', body: JSON.stringify(c.repo) }); repoId = r.id; }
  ...
  host.on($('wzForm'), 'submit', async (e) => {
    ...
    try {
      ...
      const r = await http('/deploy/setup', { method: 'POST', body: JSON.stringify({ repo: c.repo || undefined, repoId: c.repoId || undefined, target: c.target, secrets: c.secrets }) });
      if (c.repoId && manifest) await http(`/deploy/repos/${c.repoId}/manifest`, { method: 'PUT', body: JSON.stringify({ manifest }) });
      $('dpWizard').close(); toast(`${r.target ? 'Target "' + r.target.name + '" is ready' : 'Repository connected'}`, 'success');
      await loadDeploy();
      if (r.target) { dpSelect(r.target.id); if ($('wzPlanNow').checked) { const p = await http(`/deploy/targets/${r.target.id}/plan`, { method: 'POST', body: JSON.stringify({ ai: dpPrefAi() }) }); dp.runs.set(p.run.id, p.run); dp.logRun = p.run.id; dpRenderMain(); dpShowTab('log'); dpLoadLog(p.run.id); } }
  ...
  async function dpSaveTemplateFromForm() {
    ...
    try {
      if (t) await http('/deploy/templates', { method: 'POST', body: JSON.stringify({ name, fromTargetId: t.id }) });
      else { const body = dpTargetFormBody(); delete body.name; delete body.repoId; delete body.projectId; delete body.autoShip; await http('/deploy/templates', { method: 'POST', body: JSON.stringify({ name, data: body }) }); }
      toast(`Template "${name}" saved`, 'success'); dp.templates = (await http('/deploy/templates')).templates; dpFillTemplates(null);
    } catch (e) { toast(e.message, 'error'); }
  }
  function dpAskText(title, message, placeholder) {
    return new Promise((resolve) => {
      confirm({ title, message: `<p>${message}</p><input id="dpAskInput" placeholder="${esc(placeholder || '')}" style="width:100%">`, okLabel: 'Save' }).then((ok) => resolve(ok ? ($('dpAskInput')?.value || '').trim() : null));
  ...
  async function dpManageTemplates() {
    ...
    const p = confirm({ title: 'Templates', message: `<div id="dpTplList">${list}</div>`, okLabel: 'Close', cancelLabel: ' ' });
    setTimeout(() => $('dpTplList')?.querySelectorAll('[data-tpl-del]').forEach((b) => b.addEventListener('click', async () => { try { await http(`/deploy/templates/${b.dataset.tplDel}`, { method: 'DELETE' }); b.closest('.act-row').remove(); dp.templates = dp.templates.filter((x) => x.id !== b.dataset.tplDel); dpFillTemplates(dp.editingTarget); } catch (e) { toast(e.message, 'error'); } })), 50);
    await p;
  }
  async function dpDuplicateTarget(t) {
    ...
    if (!name) return;
    try { const r = await http(`/deploy/targets/${t.id}/duplicate`, { method: 'POST', body: JSON.stringify({ name, env: /stag|pre|dev|test/i.test(name) ? name.split('-').pop() : undefined }) }); toast(`Target "${r.name}" created`, 'success'); await loadDeploy(); dpSelect(r.id); }
    catch (e) { toast(e.message, 'error'); }
  }
  async function dpExportConfig() {
    try {
      const doc = await http('/deploy/export');
  ...
  host.on($('dpImportFile'), 'change', async () => { const f = $('dpImportFile').files[0]; if (f) $('dpImportText').value = await f.text(); });
  host.on($('btnDpImportCancel'), 'click', () => $('dpImportModal').close());
  async function dpRunImport(dryRun) {
    ...
    try {
      const rep = await http('/deploy/import', { method: 'POST', body: JSON.stringify({ config: doc, dryRun, projectId }) });
  ...
  host.on($('btnDpImportDry'), 'click', () => dpRunImport(true));
  host.on($('dpImportForm'), 'submit', (e) => { e.preventDefault(); dpRunImport(false); });
  ...
  function dpRenderCloudPanel() {
    ...
    el.querySelector('[data-cloud-cancel]')?.addEventListener('click', async () => { try { await http(`/deploy/cloud/jobs/${j.id}/cancel`, { method: 'POST' }); } catch (e) { toast(e.message, 'error'); } });
  ...
  function dpShowServer(x) {
    ...
    confirm({ title: `${x.name} · ${x.provider}`, okLabel: 'Close', cancelLabel: 'Add a target here', message: `<dl class="dp-kv"><dt>status</dt><dd>${esc(x.status)}${x.error ? ': ' + esc(x.error) : ''}</dd><dt>ip</dt><dd>${esc(x.ip || 'not assigned yet')}</dd><dt>region / size</dt><dd>${esc(x.region)} · ${esc(x.size)} · ${esc(x.image)}</dd><dt>recipe</dt><dd>${esc(x.recipe)}</dd><dt>ssh profile</dt><dd>${prof ? esc(prof.name) : (x.profileId ? esc(x.profileId) : 'not registered')}</dd><dt>private key</dt><dd>${esc(x.privateKeyPath || 'none')}</dd>${x.console ? `<dt>console</dt><dd><a href="${esc(x.console)}" target="_blank" rel="noopener">${esc(x.console)}</a></dd>` : ''}</dl>` })
  ...
  async function dpOpenCloudModal() {
    ...
    try { dp.cloudMeta = dp.cloudMeta || await http('/deploy/cloud/providers'); } catch (e) { return toast(e.message, 'error'); }
  ...
  host.on($('dcProvider'), 'change', () => dpCloudSync(true));
  host.on($('dcMakeTarget'), 'change', () => dpCloudSync(false));
  host.on($('dcRecipe'), 'change', () => { $('dcPreview').textContent = ''; });
  host.on($('btnDcOptions'), 'click', async () => {
    ...
    try {
      const o = await http(`/deploy/cloud/options?provider=${encodeURIComponent(provider)}&tokenRef=${encodeURIComponent(tokenRef)}`);
  ...
  $('dpCloudModal').querySelector('details').addEventListener('toggle', async (e) => {
    ...
    try { const r = await fetch('/deploy/cloud/preview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ recipe: $('dcRecipe').value, appName: $('dcTargetName').value || $('dcName').value || 'app' }) }); $('dcPreview').textContent = await r.text(); } catch (err) { $('dcPreview').textContent = err.message; }
  });
  host.on($('btnDpCloudCancel'), 'click', () => $('dpCloudModal').close());
  host.on($('dpCloudForm'), 'submit', async (e) => {
    e.preventDefault();
    ...
    if (!(await confirm({ title: 'Provision a server', message: `Create <b>${esc(body.name)}</b> on <b>${esc(p?.label || body.provider)}</b> (${esc(body.region)} · ${esc(body.size)} · ${esc(body.image)}) with the <b>${esc(body.recipe)}</b> recipe? <span class="hint">This creates billable infrastructure at the provider.</span>`, okLabel: 'Provision', okClass: 'warn' }))) return;
    try {
      const r = await http('/deploy/cloud/provision', { method: 'POST', body: JSON.stringify(body) });
  ...
  async function dpPreShipReview(t) {
    ...
    const r = await http(`/deploy/targets/${t.id}/preflight`, { method: 'POST' });
    t.preShip = r.review;
  ...
  function dpRenderOverview(t) {
    ...
    if (acceptAi) acceptAi.addEventListener('click', async (e) => { e.stopPropagation(); try { await http(`/deploy/repos/${t.repoId}/manifest`, { method: 'PUT', body: JSON.stringify({ manifest: dp.det.suggestion.fragment }) }); toast('Manifest saved', 'success'); dp.det.suggestion = null; await loadDeploy(); } catch (err) { toast(err.message, 'error'); } });
  }
  ...
  async function dpWireAutoCard(t) {
    ...
    try {
      const w = await http(`/deploy/targets/${t.id}/webhook`);
      ...
      if (w.mode === 'webhook') {
        ...
        body.querySelector('[data-poll-now]')?.addEventListener('click', async (e) => { e.stopPropagation(); try { await http(`/deploy/targets/${t.id}/poll-now`, { method: 'POST' }); toast('Polled', 'success'); dpWireAutoCard(t); } catch (err) { toast(err.message, 'error'); } });
      }
  ...
  function dpRenderMap(t) {
    ...
    $('dpMap').querySelector('[data-act="map-releases"]').addEventListener('click', async (e) => { e.stopPropagation(); try { dp.releases = await http(`/deploy/targets/${t.id}/releases`); dpRenderMap(t); } catch (err) { toast(err.message, 'error'); } });
    dpMapZoomInit(t, W, H);
  }
  ...
  host.on($('dpRunSel'), 'change', () => { dp.logRun = $('dpRunSel').value || null; dpLoadLog(dp.logRun); });
  async function dpLoadLog(runId) {
    const el = $('dpLog');
    el.innerHTML = ''; el.dataset.run = runId || ''; dp.logCursor = 0;
    if (!runId) return;
    try {
      const d = await http(`/deploy/runs/${runId}/log?since=0`);
  ...
  async function dpRenderReleases(t) {
    ...
    if (!dp.releases) {
      ...
      try { dp.releases = await http(`/deploy/targets/${t.id}/releases`); } catch (e) { el.innerHTML = `<div class="empty-block"><b>Could not list releases</b>${esc(e.message)}<br><button data-act="test">Test connection</button></div>`; return; }
  ...
  async function dpRenderSetup(t) {
    const el = $('dpSetup');
    if (!dp.setup) { try { dp.setup = await http(`/deploy/targets/${t.id}/setup`); } catch (e) { el.innerHTML = `<div class="dp-err">${esc(e.message)}</div>`; return; } if (dp.tab !== 'setup') return; }
  ...
  function dpRenderManifest(t) {
    ...
    $('dpManifestDownload').href = repo ? `/deploy/repos/${repo.id}/manifest/download` : '#';
  ...
  host.on($('dpTarget'), 'click', async (e) => {
    ...
    try {
      if (act === 'test') {
        ...
        try { const r = await http(`/deploy/targets/${t.id}/test`, { method: 'POST' }); dp.probe = { ok: true, ...r }; toast(`Connected to ${r.host}${r.canExec ? '' : ' (no shell)'}`, 'success'); }
        ...
        const r = await http(`/deploy/repos/${repo.id}/fetch`, { method: 'POST' });
        ...
        dp.det = await http(`/deploy/repos/${repo.id}/detect`, { method: 'POST', body: JSON.stringify({ ai: dpPrefAi() }) }); dp.detRepo = repo.id;
        ...
        const r = await http(`/deploy/targets/${t.id}/plan`, { method: 'POST', body: JSON.stringify({ ai: dpPrefAi() }) });
      ...
      else if (act === 'cancel') { const run = dpRunsFor(t.id)[0]; if (run) await http(`/deploy/runs/${run.id}/cancel`, { method: 'POST' }); }
        ...
        if (!(await confirm({ title: 'Delete target', message: `Delete target <b>${esc(t.name)}</b>? Nothing is removed from the server.`, okLabel: 'Delete', okClass: 'reject' }))) return;
        await http(`/deploy/targets/${t.id}`, { method: 'DELETE' }); dp.sel = null; toast('Target deleted', 'success'); await loadDeploy();
      }
    } catch (err) { toast(err.message, 'error'); }
  });
  async function dpShip(t, planHash) {
    ...
    if (dpAssist('preShipReview')) {
      ...
      catch (e) { if (!(await confirm({ title: 'Pre-ship review unavailable', message: `The AI review could not run: <b>${esc(e.message)}</b>. Continue without it?`, okLabel: 'Continue', okClass: 'warn' }))) return; }
    }
    ...
    if (!(await confirm({ title: `Ship to ${t.name}`, message: msg, okLabel: review?.verdict === 'block' ? 'Ship anyway' : 'Ship now', okClass: review?.verdict === 'block' ? 'reject' : 'warn' }))) return;
    try {
      const r = await http(`/deploy/targets/${t.id}/ship`, { method: 'POST', body: JSON.stringify({ confirm: true, planHash: planHash || undefined, ai: dpPrefAi() }) });
  ...
  async function dpRollback(t, release) {
    if (!(await confirm({ title: `Roll back ${t.name}`, message: `Switch <b>${esc(t.name)}</b> back to ${release ? `release <b>${esc(release)}</b>` : 'the <b>previous release</b>'}? The web server / process is reloaded afterwards${t.healthUrl ? ' and the health check runs' : ''}.`, okLabel: 'Roll back', okClass: 'reject' }))) return;
    try {
      const r = await http(`/deploy/targets/${t.id}/rollback`, { method: 'POST', body: JSON.stringify({ confirm: true, release: release || undefined }) });
      dp.runs.set(r.run.id, r.run); dp.logRun = r.run.id; dp.releases = null; dpRenderMain(); dpShowTab('log'); dpLoadLog(r.run.id);
    } catch (e) { toast(e.message, 'error'); }
  }
  ...
  host.on($('drKind'), 'change', dpRepoSync); host.on($('drAuth'), 'change', dpRepoSync);
  host.on($('btnDpRepoCancel'), 'click', () => $('dpRepoModal').close());
  let dpBrowseRows = [];
  ...
  host.on($('btnDrBrowse'), 'click', async () => {
    ...
    try { const r = await http(`/deploy/providers/repos?kind=${encodeURIComponent(kind)}&tokenRef=${encodeURIComponent(tokenRef)}`); dpBrowseRows = r.repos; $('drBrowse').hidden = false; $('drBrowseFilter').value = ''; dpRenderBrowse(); toast(`${r.repos.length} repositories`, 'success'); }
    catch (e) { toast(e.message, 'error'); }
    finally { $('btnDrBrowse').disabled = false; }
  });
  host.on($('drBrowseFilter'), 'input', dpRenderBrowse);
  host.on($('dpRepoForm'), 'submit', async (e) => {
    e.preventDefault();
    const kind = $('drKind').value, auth = $('drAuth').value;
    ...
    try {
      ...
      const r = await http(dp.editingRepo ? `/deploy/repos/${dp.editingRepo.id}` : '/deploy/repos', { method: dp.editingRepo ? 'PUT' : 'POST', body: JSON.stringify(body) });
  ...
  host.on($('dtRepo'), 'change', () => { tfPrefillName(); dpTargetSync(); });
  host.on($('dtHealthEnabled'), 'change', () => { if ($('dtHealthEnabled').checked) setTimeout(() => $('dtHealth').focus(), 20); });
  host.on($('dtTemplate'), 'change', () => { const v = $('dtTemplate').value; if (v === '__manage') { $('dtTemplate').value = ''; dpManageTemplates(); } else if (v) dpApplyTemplate(v); });
  host.on($('btnDtSaveTemplate'), 'click', dpSaveTemplateFromForm);
  host.on($('dtWeb'), 'change', () => { const w = $('dtWeb').value; const def = { nginx: 'sudo -n systemctl reload nginx', apache: 'sudo -n systemctl reload apache2', none: '' }; if (!$('dtReload').value || Object.values(def).includes($('dtReload').value)) $('dtReload').value = def[w] || ''; });
  ...
  host.on($('dtConnUrl'), 'change', () => tfApplyConnUrl(true));
  host.on($('dtConnUrl'), 'input', () => tfApplyConnUrl(false)); // a pasted URL applies immediately
  host.on($('dpTargetModal'), 'click', (e) => {
  ...
  hostOn($('serverFormModal'), 'close', async () => {
    if (!$('dpTargetModal').open) return;
    try { const d = await serverProfiles(); dp.profiles = d.sessions; } catch { return; }
    const profs = dp.profiles.map((p) => ({ value: p.id, label: `${p.name} (${p.user}@${p.host})` }));
  ...
  host.on($('dtPaasProvider'), 'change', () => dpPaasFields(dp.editingTarget));
  host.on($('btnDpTargetCancel'), 'click', () => $('dpTargetModal').close());
  ...
  async function tfRunTest(body) {
    ...
    try {
      const r = await http('/deploy/targets/test', { method: 'POST', body: JSON.stringify({ ...body, id: dp.editingTarget?.id || undefined }) });
  ...
  host.on($('btnDtTest'), 'click', async () => { const errs = tfValidate().filter((e) => !['dtName', 'dtRepo'].includes(e.id)); if (errs.length) return tfShowErrors(errs); await tfRunTest(dpTargetFormBody()); });
  async function tfSave(body) {
    try {
      const r = await http(dp.editingTarget ? `/deploy/targets/${dp.editingTarget.id}` : '/deploy/targets', { method: dp.editingTarget ? 'PUT' : 'POST', body: JSON.stringify(body) });
  ...
  host.on($('dpTargetForm'), 'submit', async (e) => {
    e.preventDefault();
  ...
  host.on($('btnDtSaveAnyway'), 'click', async () => { const errs = tfValidate(); if (errs.length) return tfShowErrors(errs); await tfSave(dpTargetFormBody()); });
  ...
  async function dpTargetWebhookInfo() {
    ...
    try {
      ...
      host.on($('btnDtHookReveal'), 'click', () => { const c = $('dtHookSecret'); const shown = c.textContent !== '••••••••••••••••'; c.textContent = shown ? '••••••••••••••••' : c.dataset.secret; $('btnDtHookReveal').textContent = shown ? 'Reveal' : 'Hide'; });
      host.on($('btnDtRotate'), 'click', async () => {
        const ok = await confirm({ title: 'Rotate the webhook secret', message: `Pushes signed with the current secret will be <b>rejected</b> as soon as it is rotated. Update the webhook configuration on ${esc(w.mode === 'webhook' ? 'the git provider' : 'the provider')} with the new secret afterwards.`, okLabel: 'Rotate secret', okClass: 'warn' });
        if (!ok) return;
        try {
          ...
          await http(`/deploy/targets/${t.id}`, { method: 'PUT', body: JSON.stringify(body) });
  ...
  host.on($('btnDpAdd'), 'click', (e) => { e.stopPropagation(); $('dpAddMenu').hidden = !$('dpAddMenu').hidden; });
  host.on($('dpAddMenu'), 'click', (e) => {
  ...
  host.on(document, 'click', (e) => { if (!$('dpAddMenu').hidden && !e.target.closest('.asc-add')) $('dpAddMenu').hidden = true; });
  host.on($('btnDpRefresh'), 'click', () => loadDeploy());
  host.on($('btnDpTheme'), 'click', () => host.ui.toggleTheme());
  host.on($('btnDpSettings'), 'click', () => host.ui.openSettings('deploy'));
  host.on($('btnDpClose'), 'click', closeDeploy);
  hostOn($('btnDpNew'), 'click', () => dpOpenWizard());
  ...
  return {
    openDeploy, closeDeploy, loadDeploy, dpSelect, dpOpenRepoModal, dpOpenTargetModal,
    dpRepoSync, dpFillSelect, dpFmtAgo, dpShowTab, dpLoadLog, onDeployEvent,
    renderDeploySettings: typeof renderDeploySettings === 'function' ? renderDeploySettings : () => {},
    state: dp,
  };
}
