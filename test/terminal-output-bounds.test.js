...
const { createSshAgent } = require('../lib/ssh-agent');
const { boundText, boundTerminalView, DEFAULTS } = require('../lib/model-output');
...
function setup(t, { limits = {}, modelLimits = {}, aiAssist = {} } = {}) {
  ...
  const api = createSshAgent({
    app: { get() {}, post() {} }, DATA_DIR: dir, terminals, agent, modelLimits,
    settings: { aiAssist: { sshRead: true, sshWrite: true, sshDestructive: true, sshMemory: true, ...aiAssist } },
    profileById: (id) => profiles[id], audit() {}, logEvent() {}, wrap: (fn) => fn,
    httpError: (status, message) => Object.assign(new Error(message), { status }),
  });
  t.after(async () => { await api.flush(); fs.rmSync(dir, { recursive: true, force: true }); });
  return { api, agent, terminals, shells, state };
}
...
const read = (api, agent, id, input) => api.withSession(id, () => agent.tools.ssh_terminal_read.run(input));
...
test('an approved command with a huge stdout keeps its tail and reports what was dropped', async (t) => {
  ...
  const proposal = await api.withSession(id, () => agent.tools.ssh_exec.run({ cmd: 'cat /var/log/big.log', why: 'inspect' }));
  const pending = agent.proposals.find((p) => p.id === proposal.proposalId);
  const result = await api.withSession(id, () => agent.kinds['ssh-command'].approve(pending));
...
test('a command whose capture already overflowed says so alongside the model bound', async (t) => {
  ...
  const proposal = await api.withSession(id, () => agent.tools.ssh_exec.run({ cmd: 'cat /var/log/big.log' }));
  const pending = agent.proposals.find((p) => p.id === proposal.proposalId);
  const result = await api.withSession(id, () => agent.kinds['ssh-command'].approve(pending));
