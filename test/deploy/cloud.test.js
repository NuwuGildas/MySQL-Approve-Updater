...
const { cloudInit, RECIPES } = require('../../backend/deploy/cloud/recipes');
const rest = require('../../backend/deploy/cloud/providers/rest');
const digitalocean = require('../../backend/deploy/cloud/providers/digitalocean');
const hetzner = require('../../backend/deploy/cloud/providers/hetzner');
const { PROVIDERS } = require('../../backend/deploy/cloud/providers');
const { createCloud, cloudDeps } = require('../../backend/deploy/cloud');
...
test('DigitalOcean: key reuse, droplet create with user-data, status → ready', async () => {
  const calls = fakeFetch([
    { method: 'GET', re: /account\/keys/, body: { ssh_keys: [{ id: 77, fingerprint: require('../../backend/deploy/cloud/providers/keys').fingerprintMd5(PUB) }] } },
