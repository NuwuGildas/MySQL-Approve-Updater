...
const { fakeCtx, deps, fakeConn, registerFakeVps, waitDone, fakeProjects } = require('./helpers');
const { createStores } = require('../../backend/deploy/store');
const { sanitizeRepo, sanitizeTarget, createRouter } = require('../../backend/deploy/routes');
const { createConfig } = require('../../backend/deploy/config');
...
test('legacy deployment migration preserves valid owners, uses links then General, backs up and is idempotent', async () => {
  ...
  createStores(ctx.DATA_DIR, { projects });   // a restart sees the same host projects
  for (const name of Object.keys(originals)) {
...
test('migration uses an existing project when General was removed, and stops when projects cannot be read', async () => {
  const ctx = fakeCtx();
  const projects = fakeProjects();
  ...
  /* projects.json belongs to the host now: when IT cannot read it, this module
     is told so, and it must neither migrate nor accept a new deployment. */
  ...
  const unreadable = createStores(broken.DATA_DIR, { projects: fakeProjects([], { readOnly: 'projects.json cannot be read' }) });
  assert.ok(unreadable.projects.readOnly);
  assert.equal(fs.readFileSync(path.join(broken.DATA_DIR, 'deploy-targets.json'), 'utf8'), rawTargets, 'nothing is migrated');
  ...
  assert.throws(() => sanitizeTarget({ projectId: 'general' }, broken, unreadable, null), (e) => e.status === 503);
});
