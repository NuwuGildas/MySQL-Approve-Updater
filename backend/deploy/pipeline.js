...
async function runPipeline({ ctx, run, target: rawTarget, repo, stores, vault, redact, aiDetect, previousShip = null }) {
  ...
  try {
    ...
    try {
      ...
      if (stack.remoteOnly) {
        if (!conn.canExec || target.type === 'shared-hosting' || adapter.capabilities.local) throw new Error(`${stack.label} stacks need a server with a shell (VPS target): they are built where they run`);
      ...
      if (buildWhere === 'local' && manifest.build.steps.length) {
        ...
        if (manifest.stack.type === 'node' && manifest.runtime.kind === 'node' && !adapter.capabilities.local) warn('node_modules built on this machine are shipped as-is; native modules may not match the server');
        if (manifest.stack.type === 'php' && process.platform === 'win32' && !adapter.capabilities.local) warn('composer vendor built on Windows: packages with platform-specific binaries may not run on Linux');
      }
    ...
    try {
      ...
      const stepProgress = (cmds, i, c) => { if (cmds.length > 1) run.setProgress({ label: `step ${i + 1}/${cmds.length}: ${c}`, done: i, total: cmds.length, unit: 'steps' }); else run.setProgress({ label: `$ ${c}`, done: 0, total: null }); };
      const localCmds = async (cmds, cwd) => { for (const [i, c] of cmds.entries()) { checkCancel(); sys(`$ ${c}`); stepProgress(cmds, i, c); await localExec(c, { cwd, env: buildEnv, onLine: out, signal: run.signal, timeoutMs: 45 * 60000 }); } if (cmds.length) run.setProgress(null); };
      const remoteCmds = async (cmds, cwd, extraEnv = {}) => { for (const [i, c] of cmds.entries()) { checkCancel(); sys(`$ ${c}`); stepProgress(cmds, i, c); await conn.exec(c, { cwd, env: { ...buildEnv, ...extraEnv }, onLine: out, signal: run.signal, timeoutMs: 45 * 60000 }); } if (cmds.length) run.setProgress(null); };
      ...
      if (buildWhere === 'remote') {
        ...
        releaseCreated = true;
        { let lastPct = -1; await conn.uploadFile(srcTgz, L.releaseSrcTgz(ts), (done, total) => { run.setProgress({ label: 'uploading the source', done, total, unit: 'bytes' }); const pct = total ? Math.floor((done / total) * 10) * 10 : 0; if (pct !== lastPct) { lastPct = pct; sys(`  ${artifact.fmtBytes(done)} / ${artifact.fmtBytes(total)}`); } }); run.setProgress(null); }
    ...
    if (buildWhere === 'local') {
      ...
      try {
        ...
        if (conn.canExec) {
          ...
          let lastLogged = 0;
          const p = await artifact.pack(stageDir, tgz, { total: st.files, onProgress: (n, total) => { run.setProgress({ label: 'compressing the artifact', done: n, total, unit: 'files' }); if (n - lastLogged >= 500) { lastLogged = n; sys(`  compressed ${n}${total ? '/' + total : ''} entries`); } } });
          sys(`artifact ${path.basename(tgz)} (${artifact.fmtBytes(p.size)})`);
        }
    ...
    try {
      ...
      if (conn.canExec) {
        if (buildWhere === 'local') {
          sys(`uploading artifact → ${L.releaseTgz(ts)}`); releaseCreated = true;
          let lastPct = -1;
          await adapter.extractTgz(conn, L, ts, tgz, { onLine: out, onProgress: (d, t) => { run.setProgress({ label: adapter.capabilities.local ? 'extracting the artifact' : 'uploading the artifact', done: d, total: t, unit: 'bytes' }); const pct = t ? Math.floor((d / t) * 10) * 10 : 0; if (pct !== lastPct) { lastPct = pct; sys(`  ${artifact.fmtBytes(d)} / ${artifact.fmtBytes(t)}`); } }, onEntry: (n, total) => run.setProgress({ label: 'extracting the artifact', done: n, total, unit: 'files' }) });
          run.setProgress(null);
        }
