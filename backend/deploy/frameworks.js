...
function fragmentFor(id, over = {}) {
  ...
  if (b) {
    ...
    if (b.hooks) frag.hooks = b.hooks;
    if (b.artifact) frag.artifact = b.artifact;
    if (b.build?.env && Object.keys(b.build.env).length) frag.build.env = { ...frag.build.env, ...b.build.env };
    if (b.stack?.packageManager) frag.stack.packageManager = b.stack.packageManager;
