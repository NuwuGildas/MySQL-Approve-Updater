'use strict';
/* This module's REST API, served on its own surface and proxied by the host as
   /api/m/deployments/http/deploy/*. Secrets never leave the server: targets and
...
function sanitizeTarget(body, ctx, stores, existing) {
  const err = (m) => { throw ctx.httpError(400, m); };
  const project = requireProject(stores, body.projectId === undefined ? existing?.projectId : body.projectId);
  const name = String(body.name || '').trim();
  if (!name || name.length > 80) err('name is required (max 80 chars)');
  ...
  const draft = { ...body, projectId: project.id, id: existing?.id || crypto.randomUUID(), name, repoId, overrides, createdAt: existing?.createdAt || new Date().toISOString() };
  delete draft.locked; delete draft.lastRun; delete draft.projectName;
...
function createRouter({ ctx, engine, stores, vault, redact, agentApi, autoShip, cloud, config }) {
  ...
  r.put('/targets/:id', wrap(async (req, res) => {
    ...
    if (engine.isLocked(list[i].id) || engine.activeIds().some((id) => engine.get(id)?.targetId === list[i].id)) throw httpError(409, 'a deploy is running on this target');
  ...
  /* Test a draft (unsaved or edited) target: validate through the adapter, connect, probe, and look at the deployment directory. */
  r.post('/targets/test', wrap(async (req, res) => {
    const b = req.body || {};
    const existing = b.id ? stores.findTarget(String(b.id)) : null;
    if (!TARGETS[b.type]) throw httpError(400, 'type must be one of ' + Object.keys(TARGETS).join(', '));
    const draft = { ...b, id: existing?.id || 'draft', name: String(b.name || 'draft'), repoId: String(b.repoId || existing?.repoId || '') };
    delete draft.locked; delete draft.lastRun; delete draft.preShip; delete draft.lastProbe;
    let norm;
    try { norm = adapterFor(draft).validate(draft, ctx); } catch (e) { throw httpError(e.status || 400, e.message); }
    const adapter = adapterFor(norm);
    let conn;
    ...
    try {
      ...
      let dir = null;
      const dirPath = norm.type === 'vps-ssh' || norm.type === 'local' ? norm.paths?.root : norm.type === 'shared-hosting' ? norm.paths?.docroot : null;
      if (dirPath && typeof conn.exists === 'function') { try { dir = { path: dirPath, exists: await conn.exists(dirPath), writable: norm.type === 'vps-ssh' || norm.type === 'local' ? probe.rootWritable ?? null : null }; } catch { dir = { path: dirPath, exists: null, writable: null }; } }
      res.json({ ok: true, canExec: conn.canExec, user: conn.user || norm.transport?.user || null, host: conn.host || norm.transport?.host || null, cli: norm.type === 'paas' ? TARGETS.paas.PROVIDERS[norm.paas.provider]?.cli : undefined, probe: conn.canExec ? { os: probe.os, user: probe.user, current: probe.current, releases: (probe.releases || []).length, sudo: !!probe.sudo } : null, strategy, dir });
    } finally { conn.close(); }
  }));
...
# Give the deploy user docker access:  sudo usermod -aG docker ${user}`; }
      ...
      const webroot = adapter.webRoot(L, m);
      out['serve it on this computer'] = `# Each deploy swaps "current" to the new release; rollback swaps it back.
# Web root to serve: ${webroot}
${m.runtime.kind === 'php-fpm' ? `# PHP built-in server:   php -S 127.0.0.1:8080 -t "${webroot}"\n# XAMPP / Laragon / nginx / IIS: point the document root at the path above` : ''}
${m.runtime.kind === 'static' ? `# Quick static server:   npx serve "${webroot}"\n# or point XAMPP / nginx / IIS at the path above` : ''}
${m.runtime.kind === 'node' || m.runtime.kind === 'python' ? `# Start command (runs in ${L.current}): ${m.runtime.start || '(set runtime.start in the manifest)'}\n# Set the process manager to pm2 so each deploy restarts it, or use "After activation, run".` : ''}
# Health check URL for this target: ${norm.healthUrl || 'none (add one, e.g. http://localhost:8080/)'}`;
  ...
  /* Wizard "AI detect": clone (or read) the source, run heuristics, ask the AI to confirm/refine, map to the catalog + form.
     Draft git sources are cloned into workDir/_drafts and removed afterwards; a pasted token is used once and never stored. */
  const detectCache = new Map(); // source key → { at, result }: the same source is not cloned or sent to the AI again for a while
  const DETECT_TTL_MS = 30 * 60000;
  r.post('/detect-source', wrap(async (req, res) => {
    const fs = require('fs'), fsp = require('fs/promises');
    const b = req.body || {};
    const useAi = b.ai !== false;
    if (useAi && !ctx.agent.isConnected()) throw httpError(409, 'connect the AI assistant first (Settings → AI assistant)');
    let source = b.source || null, repo = null, label = null, dir, cleanup = null;
    if (b.repoId) { repo = stores.findRepo(String(b.repoId)); if (!repo) throw httpError(404, 'repo not found'); source = repo.source; label = repo.name; }
    if (!source || !['git', 'local'].includes(source.kind)) throw httpError(400, 'source.kind must be git or local');
    const cacheKey = JSON.stringify(b.repoId ? { repoId: String(b.repoId) } : source.kind === 'local' ? { path: String(source.path || '') } : { url: String(source.url || '').trim(), branch: source.branch || null }) + '|ai:' + useAi;
    for (const [k, v] of detectCache) if (Date.now() - v.at > DETECT_TTL_MS) detectCache.delete(k);
    const hit = !b.force && detectCache.get(cacheKey);
    if (hit) return res.json({ ...hit.result, cached: true, cachedAt: new Date(hit.at).toISOString() });
    if (source.kind === 'local') {
      dir = String(source.path || '');
      if (!dir || !path.isAbsolute(dir) || !fs.existsSync(dir)) throw httpError(400, 'path must be an existing absolute folder');
      label = label || path.basename(dir);
    } else {
      const url = String(source.url || '').trim();
      if (!/^(https?:\/\/|ssh:\/\/|git@)[^\s]+$/.test(url)) throw httpError(400, 'repository URL must be http(s), ssh:// or git@host:path');
      label = label || url.replace(/\.git$/, '').split(/[\/:]/).pop();
      let token = null;
      if (repo) token = repo.source.auth?.kind === 'https-token' ? vault.get(refName(repo.source.auth.tokenRef)) : null;
      else if (source.auth?.kind === 'https-token') token = source.auth.token ? String(source.auth.token) : (source.auth.tokenRef && isRef(source.auth.tokenRef) ? vault.get(refName(source.auth.tokenRef)) : null);
      const src = { kind: 'git', url, branch: source.branch || null, auth: source.auth?.kind === 'ssh' ? { kind: 'ssh', keyPath: source.auth.keyPath || null } : null };
      if (repo) dir = path.join(stores.workDir, repo.id, 'src');
      else { const key = crypto.createHash('sha1').update(url + '\n' + (src.branch || '')).digest('hex').slice(0, 16); cleanup = path.join(stores.workDir, '_drafts', key); dir = path.join(cleanup, 'src'); }
      const lines = [];
      try { const info = await git.sync(src, dir, { token, onLine: (l) => lines.push(redact(l)) }); if (repo) { repo.lastFetch = { commit: info.commit, branch: info.branch, at: new Date().toISOString(), subject: info.subject }; await stores.repos.save(); } }
      catch (e) { if (cleanup) await fsp.rm(cleanup, { recursive: true, force: true }).catch(() => {}); throw httpError(400, `git clone failed: ${redact(e.message)}${lines.length ? ': ' + lines.slice(-3).join(' | ') : ''}`); }
    }
    try {
      const det = await detect(dir);
      let suggestion = null;
      if (useAi && agentApi?.aiDetect) {
        suggestion = await agentApi.aiDetect(det);
        audit({ action: 'deploy-ai-detect', repo: label, confidence: suggestion?.confidence ?? null, ok: !!suggestion, wizard: true });
      }
      const fragment = suggestion?.fragment || det.best?.fragment || null;
      let resolved = null; try { resolved = fragment ? manifestLib.validate(fragment) : null; } catch {}
      const catalogId = frameworks.catalogIdFor(fragment);
      const result = {
        label, by: suggestion ? 'ai' : det.best ? 'heuristic' : null, aiFailed: useAi && !suggestion,
        confidence: suggestion ? suggestion.confidence : det.best ? det.best.score : null, reasoning: suggestion?.reasoning || null,
        catalogId, form: resolved ? frameworks.formFrom(resolved) : null, fragment: resolved ? manifestLib.compact(resolved) : null,
        best: det.best ? { id: det.best.id, label: det.best.label, root: det.best.root, score: det.best.score, evidence: det.best.evidence } : null,
        candidates: det.candidates.map((c) => ({ id: c.id, root: c.root, score: c.score, evidence: c.evidence })), ambiguous: det.ambiguous, reason: det.reason, shipJson: !!det.shipJson,
      };
      if (result.catalogId || result.by) detectCache.set(cacheKey, { at: Date.now(), result });
      res.json({ ...result, cached: false });
    } finally { if (cleanup) await fsp.rm(cleanup, { recursive: true, force: true }).catch(() => {}); }
  }));
  ...
  r.post('/import', wrap(async (req, res) => { const doc = req.body?.config || req.body; const report = await config.importConfig(doc, { dryRun: req.body?.dryRun === true || req.query.dryRun === '1', projectId: req.body?.projectId }); autoRefresh(); res.json(report); }));

  /* ---- runs ---- */
  r.get('/runs', wrap(async (req, res) => {
    const project = req.query.projectId === undefined ? null : requireProject(stores, req.query.projectId);
    const active = engine.activeIds().filter((id) => !project || engine.get(id)?.projectId === project.id);
    res.json({ runs: engine.list({ targetId: req.query.target || undefined, projectId: project?.id, limit: Math.min(200, Number(req.query.limit) || 50) }), active });
  }));
