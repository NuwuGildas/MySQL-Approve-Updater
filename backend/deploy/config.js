...
const TEMPLATE_STRIP = ['id', 'name', 'repoId', 'projectId', 'projectName', 'createdAt', 'locked', 'lastRun', 'autoShip'];
...
function suggestTarget({ kind, name, repoName, env, web = 'nginx', stackType, host }) {
  ...
  if (kind === 'local') return { ...base, type: 'local', buildMode: 'local', paths: { root: require('path').join(require('os').homedir(), 'www', slug) }, web: { reloadCmd: '' }, process: { manager: 'none', name: '' }, keepReleases: 3 };
...
function createConfig({ ctx, stores, vault, sanitizeRepo, sanitizeTarget }) {
  ...
  async function saveTemplate({ name, fromTargetId, data }) {
    ...
    payload = { ...payload }; delete payload.projectId; delete payload.projectName;
  ...
  async function duplicateTarget(id, { name, repoId, env, projectId }) {
    const src = stores.findTarget(id); if (!src) throw ctx.httpError(404, 'target not found');
    const body = { ...templateDataFrom(src), projectId: projectId === undefined ? src.projectId : projectId, name: String(name || `${src.name}-copy`).trim(), repoId: repoId || src.repoId };
    if (env && (body.type === 'vps-ssh' || body.type === 'local')) body.paths = { root: `${body.paths.root}-${env}`.replace(/-prod(uction)?-/, '-') };
  ...
  async function setup(body) {
    if (body.target) requireProject(stores, body.target.projectId);
    ...
    if (target) { requireProject(stores, target.projectId); stores.targets.get().targets.push(target); await stores.targets.save(); ctx.audit({ action: 'deploy-target-add', target: target.name, projectId: target.projectId, type: target.type, by: 'setup' }); }
   ...
   */
  async function importConfig(doc, { dryRun = false, projectId } = {}) {
    if (!doc || doc.format !== 'ascension-config') throw ctx.httpError(400, 'not an Ascension configuration export');
    if (projectId !== undefined) requireProject(stores, projectId);
    ...
    if (!dryRun) {
      ...
      for (const target of newTargets) requireProject(stores, target.projectId);
