...
function createAutoShip({ ctx, engine, stores, vault }) {
  ...
  const targetsWith = (mode) => stores.targets.get().targets.filter((t) => (!ctx.modules || ctx.modules.has('deployments')) && t.autoShip?.enabled && (!mode || t.autoShip.mode === mode));
  ...
  async function pollOnce(target) {
    if (ctx.modules && !ctx.modules.has('deployments')) return;
