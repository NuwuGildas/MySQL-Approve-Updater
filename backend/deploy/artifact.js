...
async function stage(srcDir, stageDir, manifest, meta, { onLine, onProgress } = {}) {
  ...
  for (const rel of files) {
    ...
    if (onProgress && (++i % 25 === 0 || i === files.length)) onProgress(i, files.length, bytes);
  }
...
async function pack(stageDir, outFile, { onProgress, total } = {}) {
  ...
  await tar.c({
    ...
    filter(p, st) {
      // Executable-looking files get 0755. Only the permission bits may change: overwriting the whole mode
      // drops the file-type bits and node-tar then waits forever on that entry (a ship "stuck" in package).
      if (st && st.isFile() && EXEC_RE.test(p.replace(/\\/g, '/'))) st.mode = (st.mode & ~0o777) | 0o755;
      if (onProgress && (++n % 50 === 0)) onProgress(n, total || null);
      return true;
    },
  }, ['.']);
  if (onProgress) onProgress(n, total || n);
