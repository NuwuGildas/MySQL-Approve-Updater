...
**The assistant in a server terminal.** *Connect with AI chat* on a server card opens a **shared shell**: one persistent SSH session that you and the assistant both use, and that keeps running while you close the view. The console shows it live, with a session picker, *New session* and *End session*. Because it is a real shell, state carries between commands (a `cd` or an `export` sticks) and you watch every keystroke the assistant makes. Several views of one session stay in step, and each session carries its own conversation.

Control is explicit. You hold it by default; *Give the assistant control* hands it over after a probe confirms the shell is at a prompt, and while the assistant holds it your typing is disabled. *Take back control* interrupts whatever is running. The assistant may read what is already on screen, but it can never type: **every** command, even `ls`, arrives as an approval card showing the exact line, and you choose Accept, Reject or Alternative. Accept runs it once and the assistant then interprets the result in the same turn; Reject and Alternative resume planning without running anything, and every further command needs its own approval. An approval is sealed to the terminal's revision, so if the terminal moved on in between it is refused and the assistant must propose again. Commands the classifier cannot reason about (shell loops, substitutions, pipes into a shell) and interactive programs are refused outright rather than guessed at.

**The assistant is not only a terminal feature.** With no server selected it works across the app as it always has, in the active project's conversation: rules, SQL, the schema, deployments and history, with no shell tools offered at all. Connecting it to a server switches it to that terminal's own conversation and adds the ssh tools; leaving the terminal returns to the project one. Each keeps its own history.

**Settings → AI assistant → Server access** decides which classes may be approved at all: read, write, destructive, sudo, plus per-session memory. All are off until you turn them on. An ended session keeps its transcript: it opens read-only, with no SSH connection and no new turns.

**What the assistant can reach.** The CLI providers are run against a managed home under `agent-home/`, with MCP servers, web search, hooks and their own file and shell tools disabled, a clean environment, and the app's own data files out of their working directory. If the installed CLI is too old to honour those switches, that provider's turns are refused with an explanation rather than run unrestricted. Long terminal output is bounded before it reaches the model, keeping the most recent lines, because that is where a failure states what went wrong; the terminal's own replay buffer is untouched, so what you scroll is complete.

**What is recorded.** The audit timeline holds what happened, never what was said: commands, classes, exit codes and decisions, with message text reduced to a length. Live event streams and history reads are scoped to one session, so one terminal's conversation cannot appear in another's view. Lives in `lib/ssh-terminal.js`, `lib/ssh-agent.js`, `lib/agent-workflow.js`, `lib/agent-isolation.js` and `lib/model-output.js`.

**Checking it end to end.** `test/fixtures/ssh-server.js` starts a disposable SSH server backed by a real local shell, so the regressions never touch a live box: `node test/e2e/session-workflow.js` covers sessions, viewers, scoping and invalid ids, and `node test/e2e/assistant-decisions.js` covers the approval flow with real model turns. Both create and remove their own throwaway connection profiles.

**Global search (Ctrl K).** The header search button, or Ctrl/⌘ K anywhere, opens one field over every page, action and resource. Free text searches pages ("Go to"), actions, projects, deployment targets, repositories, servers, connectors, vault secrets, database connections, deploy runs, tables and settings sections, with the matched text highlighted. Typing a prefix scopes the search to one kind: `projects:`, `targets:`, `repos:`, `servers:`, `connectors:`, `secrets:`, `connections:`, `runs:`, `tables:`, `settings:`. Arrow keys move, Enter opens, Tab completes a tip, Backspace at the start (or Escape) leaves the scope, Escape closes. The six most recent picks come back on the next open. Sources are fetched lazily, cached for 20 seconds and matched in the browser, so nothing is sent anywhere. Lives in `public/palette.js`.

**Live progress.** While a run is active, the pipeline footer shows the current stage with its elapsed time, a progress bar for the long silent steps (copying and compressing the artifact, uploading, extracting, build step N of M), the last log line and how long ago the run last gave a sign of life. After 25 s of silence the line turns amber, after 2 minutes it turns red and suggests checking the Log tab or cancelling. Runs expose `progress` and `lastActivityAt` on `GET /api/deploy/runs/:id` and in the `run` SSE events.

**Development server.** `npm run dev` runs `scripts/dev.js`, which restarts the server only when `server.js`, `lib/` or `package.json` actually change (size or mtime). Avoid plain `node --watch` on Windows: it also restarts on last-access-time updates of loaded modules (NTFS "System Managed" access times are on by default), so an IDE re-indexing, an antivirus scan or the search indexer reading a file kills the server, and with it any deploy in flight. Runs interrupted that way are marked "interrupted: the server restarted" on the next start and their lock on a This-computer target is released; crashes are recorded in `crash.log`.

**Deploying to this computer.** The wizard's destination step (and the target form) offers "This computer": a folder on the machine running Server Tools gets the same `releases/<ts>`, `current` and `shared/` layout as a VPS. It is implemented with Node's file system, so it works on Windows (directory junctions for `current` and shared dirs; shared files as symlinks, hard links or copies) as well as macOS and Linux. Builds always run locally, hooks run in the local shell, an optional "after activation" command can restart a local web server, and pm2 is supported for Node/Python servers. Point XAMPP, nginx, IIS or `php -S` at `<folder>/current/<web root>`; the Setup tab prints the exact commands. Docker stacks are not supported on this target.

**AI detect in the wizard.** The framework step of the guided setup has an "AI detect" button (enabled when the AI assistant is connected). The server clones the git URL (or reads the local folder / the saved repository), runs the heuristic detector, asks the AI to confirm and refine the result from the tree and key files, and the wizard picks the framework card and fills install/build/start, port, output directory, web root and health path. Shared paths, hooks and env from the detection are kept in the saved manifest. Draft clones live in `deploy-work/_drafts` and are removed right after; a pasted token is used for that clone only.
