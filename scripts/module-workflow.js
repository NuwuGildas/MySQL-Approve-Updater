...
jobs:
  ...
  # because a tag push only runs workflows present in the TAGGED commit - and
  ...
  #
  # Deliberately no concurrency group. GitHub cancels the superseded runs in a
  # group, which paints them red and hides real failures; publish-catalog.sh
  # checks what it actually published instead, so several modules released at
  # once converge on a complete catalog whatever order they finish in.
  catalog:
    needs: build
    if: startsWith(github.ref, 'refs/tags/${id}-v')
    runs-on: ubuntu-latest
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          ...
          cache: npm
      - run: npm ci
      - name: Rebuild and publish the catalog
        env:
          GH_TOKEN: \${{ github.token }}
        run: bash scripts/publish-catalog.sh
`;
